---
title: "Day 1A Activity (Exploratory Data Analysis) - Solutions"
output:
  html_notebook:
    df_print: paged
editor_options: 
...
trainIndex <- createDataPartition(globalWarm$govact, p = .8,
                                  list = FALSE, 
                                  times = 1)
globalWarm_train <- globalWarm[trainIndex, ]
globalWarm_test <- globalWarm[-trainIndex, ]
...
```{r}
print(dfSummary(globalWarm_train), method = 'render')
```

# EDA: Missing Data 

As you can see, the dataset does not have any missing values. For the purposes of this exercise, let's insert some missingness to explore by running these lines of code:
...
Now, use data visualizations to inspect missingness in the 'globalWarm_train_m' dataset. Specifically, 

7. How much missing data is there? (hint: aggregation plot)
8. Do missing data systematically vary by other observed data? (hint: spinograms for numeric variables)
9. Are there any patterns of missingness across the entire dataset? (hint: parallel coordinate plots)

```{r}
aggr(globalWarm_train_m, numbers = TRUE, prop = c(TRUE, FALSE), col = c("bisque", "darkcyan"))
spineMiss(globalWarm_train_m[, c("govact", "posemot")], miss.labels = FALSE)
```
...
corrplot(cormat, type = 'lower', tl.cex = 0.8)
```
