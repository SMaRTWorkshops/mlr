...
We can practice setting up tuning grids and also learn to compare models

.footnote[Note that this is a relatively small dataset with few predictors, so we may not really need the power of SVM.]
...
surv_train <- titanic[index, ]
surv_test <- titanic[-index, ]
```
...
---
class: onecol
## Training Performance of the Linear SVM
...
---
class: onecol
## Training Performance of the Radial SVM
...
```{r, echo=FALSE}
surv_rbfsvm$results %>% kable(digits = 3)
```

---
class: onecol
## Plotting Accuracy Against $C$ Values
...
---
class: onecol
## Comparing the Algorithms' Training Performance

```{r resamples, out.width='80%'}
resamples(list(LogisticGLM = surv_glm, 
               LinearSVM = surv_linsvm, 
               RadialSVM = surv_rbfsvm)) %>% bwplot()
```
...
glm_pred <- predict(surv_glm, newdata = surv_test)
linsvm_pred <- predict(surv_linsvm, newdata = surv_test)
rbfsvm_pred <- predict(surv_rbfsvm, newdata = surv_test)
...
```{r}
# Logistic Regression
postResample(pred = glm_pred, obs = surv_true)
```
...
```{r}
# Linear SVM
postResample(pred = linsvm_pred, obs = surv_true)
```
...
```{r}
# Radial SVM
postResample(pred = rbfsvm_pred, obs = surv_true)
```
