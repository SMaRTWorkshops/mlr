...
The **ultimate goal** of exploratory data analysis is to ![:emphasize](gain insights into your data in order to make informed modeling decisions). 
...
If modeling decisions are made based on data in the test set, we ![:emphasize](risk artificially inflating model performance) estimates in the test set. 

Thus, **when possible, it is ideal** to perform exploratory data analysis only on your training data only.
...
Parallel coordinate plots allow us to look at patterns of misingness across the entire dataset.

```{r, message=FALSE, warning=FALSE, results = 'hide', out.height="90%", out.width="90%"}
...
Note to self: correlation matrices
