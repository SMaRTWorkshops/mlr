...
```{r, echo = FALSE, fig.showtext = TRUE, out.width="80%"}
par(bg = "#E9ECEF")
...
#ggcorr(msq[, c(2, 4, 5, 15, 19, 20, 30, 43, 49, 55, 67)], hjust = 0.75)
par(bg = "#E9ECEF")
...
```{r, echo = FALSE}
par(bg = "#E9ECEF")
corrplot(cormat, tl.col = '#23395b', type = 'lower', method = 'number', tl.cex = 0.8, pch.cex = 0.5, number.cex = 0.55)
```
...
```{r, echo = FALSE}
par(bg = "#E9ECEF")
corrplot(cormat, tl.col = '#23395b', order = 'hclust', addrect = 3, tl.cex = 0.8, pch.cex = 0.5, number.cex = 0.55)
```

---
##What type of covariation occurs between my variables?

**Scatterplot matrices** also provide visualizations of bivariate relationships among variables.
...
data(affect)
affect <- rename(affect, neuroticism = neur, positiveAffect = PA1)
```
...
featurePlot(x = affect[, c(4, 8, 12, 20)],
            y = affect$NA1,
            plot = "scatter",
            type = c("p", "smooth"),
            labels = c("Feature", "Negative Affect", "test"))
```

.footnote[
see more at https://topepo.github.io/caret/visualizations.html
]
