...
EDsim <- EDsim %>% mutate(race = as.factor(floor(race)), age = age + 14, SES = as.factor(floor(SES)), 
                 num_siblings = floor(num_siblings), num_cellphones = floor(num_cellphones), num_pets = floor(num_pets))
```
...
Step 1: Exploratory Data Analysis
```{r, echo = FALSE}
cormat <- cor(EDsim[, sapply(EDsim_train, is.numeric)])
par(bg = "#E9ECEF")
