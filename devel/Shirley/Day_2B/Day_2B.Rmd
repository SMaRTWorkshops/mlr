...
correlated_relationship <- rnorm(n = 25, mean = .6, sd = .2)
...
nn <- corpcor::make.positive.definite(covMat, tol=1e-3)
...
preproc
EDsim_train_tf <- predict(preproc, newdata = EDsim_train)
```
...
```{r, echo = FALSE}
print(dfSummary(EDsim_train_tf, valid.col = FALSE, headings = FALSE, style = 'grid', 
                plain.ascii = FALSE, graph.magnif = 0.85, tmp.img.dir = "/tmp"),
      max.tbl.height = 300, method = 'render')
```
...
```{r}
set.seed(2021)
ridgefit <- train(ED_severity ~ ., data = EDsim_train_tf[, -2],
                  method = 'glmnet',
                  tuneGrid = ridgegrid,
...
First we need to standardize the training data:

```{r}
preproc_test <- preProcess(EDsim_test[, -c(1, 2)], method = c("center", "scale"))
preproc_test
EDsim_test_tf <- predict(preproc_test, newdata = EDsim_test)
```
...
```{r}
ridgepred_test <- predict(ridgefit, EDsim_test_tf)
postResample(pred = ridgepred_test, obs = EDsim_test_tf$ED_severity)
```

--

Let's compare this to the performance metrics on our training set: 

```{r}
ridgepred_train <- predict(ridgefit, EDsim_train_tf)
postResample(pred = ridgepred_train, obs = EDsim_train_tf$ED_severity)
```
...
**Split into a training (including cross-validation) and held-out test set.**

This time, we'll predict a categorical outcome.

```{r, out.width="50%"}
# use caret for simple data split
library(caret)
set.seed(2021)
trainIndex <- createDataPartition(EDsim$ED_diagnosis, p = 0.8, list = FALSE, times = 1)
EDsim_train_cat <- EDsim[trainIndex, ]
EDsim_test_cat <- EDsim[-trainIndex, ]
```
...
```{r}
preproc_cat <- preProcess(EDsim_train_cat[, -c(1, 2)], method = c("center", "scale"))
preproc_cat
EDsim_train_tf_cat <- predict(preproc, newdata = EDsim_train_cat)
```

---
## Lasso Regression in R

**Set model training and tuning methods**

```{r}
## custom summary function 
MySummary <- function(data, lev = NULL, model = NULL){
  a1 <- twoClassSummary(data, lev, model) # for AUC, sensitivity, specificity
  b1 <- prSummary(data, lev, model) # for PPV, AUPRC
  out <- c(a1, b1)
  out}

EDsim_control_cat <- trainControl(method = 'repeatedcv',
                                  number = 10,
                                  repeats = 3,
                                  classProbs = TRUE,
...
```{r}
set.seed(2021)
lassofit <- train(ED_diagnosis ~ ., data = EDsim_train_tf_cat[, -1],
                  method = 'glmnet',
                  family = "binomial",
                  tuneGrid = lassogrid,
                  trControl = EDsim_control_cat)
```
...
**Standardize and predict on held-out test set**

```{r}
preproc_test_cat <- preProcess(EDsim_test_cat[, -c(1, 2)], method = c("center", "scale"))
preproc_test_cat
EDsim_test_tf_cat <- predict(preproc_test_cat, newdata = EDsim_test_cat)
```
...
```{r}
lassopred_test <- predict(lassofit, EDsim_test_tf_cat)
pROC::auc(EDsim_test_tf_cat$ED_diagnosis, as.numeric(lassopred_test))
```
...
```{r}
lassopred_train <- predict(lassofit, EDsim_train_tf_cat)
pROC::auc(EDsim_train_tf_cat$ED_diagnosis, as.numeric(lassopred_train))
```
...
do regular regression, ridge, lasso, elastic net, and compare the models 

also show categorical

remember to add in some knowledge checks
