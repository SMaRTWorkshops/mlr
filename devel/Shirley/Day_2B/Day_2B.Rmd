...
set.seed(2021)
lassofit <- train(ED_severity ~ ., data = EDsim_train_tf[, -2],
                  method = 'glmnet',
                  tuneGrid = lassogrid,
                  trControl = EDsim_control)
```
...
```{r}
lassopred_test <- predict(lassofit, EDsim_test_tf)
postResample(pred = lassopred_test, obs = EDsim_test_tf$ED_severity)
```
...
```{r}
lassopred_train <- predict(lassofit, EDsim_train_tf)
postResample(pred = lassopred_train, obs = EDsim_train_tf$ED_severity)
```
