...
uncorrelated_relationship[uncorrelated_relationship > 1] <- .2
emptyCorMat[3:12, 8:12] <- uncorrelated_relationship
emptyCorMat

no_relationship <- rnorm(n = 8*20, mean = 0, sd = .1)
emptyCorMat[,13:20] <- no_relationship

firstRowCor <- rnorm(n = 10, mean = .5, sd = .2)
firstRowCor[firstRowCor>1] <- .82
emptyCorMat[1,3:12] <- firstRowCor

secondRowCor <- rnorm(n = 10, mean = .5, sd = .2)
...
stddev <- abs(rnorm(20,mean = 1, sd = 1))
...
EDsim <- EDsim %>% mutate(ED_diagnosis = as.factor(ED_diagnosis), race = as.factor(floor(race)), age = age + 14, SES = as.factor(floor(SES)), num_siblings = floor(num_siblings), num_cellphones = floor(num_cellphones), num_pets = floor(num_pets))
```
