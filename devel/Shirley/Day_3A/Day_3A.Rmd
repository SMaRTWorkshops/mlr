...
Today, we'll work with a simulated dataset examining predictors of eating disorder severity (for regression problems) and diagnosis (for classification problems)<sup>1</sup>. Simulated variables include  theoretically *related* and theoretically *unrelated* to eating disorders severity and diagnoses:
...
.footnote[
[1] Note: Slides will offer a regression example and live coding will offer a classification example.
]
...
```{r, echo = FALSE}
training <- c(summary(ridgefit$results))
```
