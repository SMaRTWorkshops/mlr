...
# Live Coding 
Goal: Classify likelihood of having a heart attack based on demographics (age, sex) and health variables (type of chest pain, resting blood pressure, cholesterol, fasting blood sugar, resting electrocardiographic results, maximum heart rate, exercise induced angina, etc.). Compare performance of standard logistic, ridge, lasso, and elastic net models. 
...
# first we set the seeds for reproducible results
set.seed(2021)
seeds <- vector(mode = "list", length = 15) # length = n data splits
for(i in 1:15) seeds[[i]] <- sample.int(1000, 21)
...
## Fit Models
```{r}
logfit <- train(heart_recipe, data = heart_train,
                method = 'glm', 
                family = "binomial",
                metric = 'ROC',
...
## Make Predictions
```{r}
logpred_test <- predict(logfit, newdata = heart_test)
ridgepred_test <- predict(ridgefit, newdata = heart_test)
lassopred_test <- predict(lassofit, newdata = heart_test)
enpred_test <- predict(enfit, newdata = heart_test)

roc(predictor = as.numeric(logpred_test), response = heart_test_baked$output)
roc(predictor = as.numeric(ridgepred_test), response = heart_test_baked$output)
roc(predictor = as.numeric(lassopred_test), response = heart_test_baked$output)
roc(predictor = as.numeric(enpred_test), response = heart_test_baked$output)
```
...
## Load data
Read in the dataset "hcp_memory.csv". 
...
## Training Methods
Set your model training and tuning methods using trainControl(). Use 10-fold internal cross-validation (no repeats) and set seeds to use for resampling during the model training process so your results are fully reproducible.
