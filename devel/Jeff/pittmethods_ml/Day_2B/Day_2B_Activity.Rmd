...
```{r}
# Combine the trusted labels and predictions into a data frame
fare_test_data <- data.frame(
  obs = fare_test_baked$fare, 
  pred = fare_pred
)

library(yardstick)
ccc(fare_test_data, truth = obs, estimate = pred)
huber_loss(fare_test_data, truth = obs, estimate = pred)

my_metrics <- metric_set(rmse, huber_loss, rsq, ccc)
my_metrics(fare_test_data, truth = obs, estimate = pred)
```
...
yardstick::roc_curve(test_data, yes, truth = obs, event_level = "second") %>% autoplot()
```
