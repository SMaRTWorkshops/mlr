...
The ![:emphasize](log), **square root**, or **inverse** transformations can help with skew

```{r logfare, echo=FALSE}
p1 <- 
ggplot(titanic, aes(x = fare)) + 
  geom_density(size = 1.25, alpha = 0.4, color = "grey40", fill = "#91bfdb") + 
  theme_xaringan(text_font_size = 16, title_font_size = 18) +
  theme(
    legend.position = "bottom",
    panel.grid.minor = element_blank(), 
...
```{r}
# There are steps for all of these transformations (and more)
log_recipe <- 
  titanic %>% 
  recipe(survived ~ .) %>% 
...
.footnote[[1] Note that, with `offset`, we are adding 1 to each value before taking the log because the log of 0 is undefined.<br />[2] There are similar functions for the other transformations, e.g., `step_sqrt()` and `step_inverse()`.]
