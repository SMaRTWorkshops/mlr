...
.footnote[[1] If you would like to use specific transformations, use: `step_log()`, `step_inverse()`, `step_sqrt()`, etc.<br />[2] As with normalizing, use `prep` to estimate λ from training set and use it when you `bake` the testing set.]
...
[2] Note that higher thresholds (e.g., 0.95) are often used with algorithms that can handle many predictors.<br />
...
Algorithm | Prediction | Interpretation
:-------- | :------------------ | :----
Linear Model | `dummy`, `nzv`, `corr`, `lincomb`, <br />(`impute_knn`,`interact`, `poly`, `pca`, `YeoJohnson`) | (`normalize`)
Regularized<br />Linear Model | `dummy`, (`impute_knn`, `interact`, `poly`, `pca`, `YeoJohnson`) | `corr`, `lincomb`, (`normalize`)
Decision Tree | | `corr`, `lincomb`, (`normalize`)
Random Forest | (`nzv`) | `corr`, `lincomb`, (`normalize`)

.footnote[
[1] `poly`, `impute_knn`, `YeoJohnson`, and `pca` are just exemplars and can be replaced by an alternative function.<br />
[2] Kuhn & Johnson (2020). *Feature Engineering and Selection: A Practical Approach for Predictive Models.* CRC Press.<br />
]
