...
---
class: onecol
## Examples of predictor Encodings
.left-column[
<br />
...
---
class: onecol
## Issues to Navigate in predictor Engineering
.left-column[
<br />
```{r, echo=FALSE}
...
# Add calculated predictors using base R
...
```{r}
# Add calculated predictors using tidyverse
library(tidyverse)
irisTrain <- irisTrain %>% 
  mutate(
...
---
class: onecol
## Recipes for predictor Engineering
...
<p style="padding-top:25px;">Now we are ready to add some preprocessing (i.e., predictor engineering) steps to the recipe!</p>
...
bake(it_recipe, new_data = etitanic_train) %>% glimpse()
```
...
x <- etitanic$age
p1 <- ggplot(tibble(x), aes(x)) + geom_density() +
  annotate(
    "label", 
    x = mean(x), 
    y = 0.005, 
    label = glue::glue("M={round(mean(x), 1)}, SD={round(sd(x), 1)}"),
    size = 5
    ) +
...
x_c <- x - mean(x)
p2 <- ggplot(tibble(x_c), aes(x_c)) + geom_density() + 
  annotate(
    "label", 
    x = mean(x_c), 
    y = 0.005, 
    label = glue::glue("M={round(mean(x_c), 1)}, SD={round(sd(x_c), 1)}"),
    size = 5
    ) +
...
x_s <- x / sd(x)
p3 <- ggplot(tibble(x_s), aes(x_s)) + geom_density() + 
  annotate(
    "label", 
    x = mean(x_s), 
    y = 0.075, 
    label = glue::glue("M={round(mean(x_s), 1)}, SD={round(sd(x_s), 1)}"),
    size = 5
    ) +
...
x_n <- (x - mean(x)) / sd(x)
p4 <- ggplot(tibble(x_n), aes(x_n)) + geom_density() + 
  annotate(
    "label", 
    x = mean(x_n), 
    y = 0.075, 
    label = glue::glue("M={round(mean(x_n), 1)}, SD={round(sd(x_n), 1)}"),
    size = 5
    ) +
...
norm_test <- bake(norm_recipe, new_data = etitanic_test)
```
...
To learn more about predictor engineering, see [1] and the [recipes](https://recipes.tidymodels.org/) package

.footnote[[1] Kuhn & Johnson (2020). *predictor Engineering and Selection: A Practical Approach for Predictive Models.* CRC Press.]
