...
- We will use several functions from {tidyverse} so install and load it to follow along
...
Numbers can just be typed but text (called **strings**) have to be quoted
...
It takes an object and passes it to a function as the first argument
...
```{r}
# Take x and then round it to 2 digits
x %>% round(digits = 2)
```
...
my_numbers
my_books <- c("Annihilation", "Catch 22", "Goblet of Fire", "Who moved my cheese?")
my_books
```

Extract parts (i.e., elements) from a vector using `[]`
...
If there are only a few values data could take on, we can tell R with `factor()`
...
This is called a **data frame** and can be used like a table or spreadsheet
...
We can extract a single variable from a data frame using `$`
...
In this course, we will be importing data from files stored online (but this also works for local files)

We will use the `read_csv()` function from {tidyverse} and give it a URL (i.e., web address) for the file
