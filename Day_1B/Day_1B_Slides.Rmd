...
<<<<<<< HEAD
In this course, we will be importing data from files stored online (but this also works for local files). We can use the 'read.csv()' function from base R or the `read_csv()` function from {tidyverse} and give it a URL for the file.
=======
In this course, we will be importing data from files stored online (but this also works for local files)

We can use `read.csv()` from base R or `read_csv()` from {tidyverse} and give it a URL for the file
>>>>>>> bbc19835d9dacd9d9b2a0fc4658c47f46dba918a
