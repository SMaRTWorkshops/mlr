...
set.seed(2022)
x <- rnorm(200)
ylin <- 1 + 4*x + 0.5*rnorm(200)
lin_df <- data.frame(y = ylin, x = x)
ggplot(lin_df, aes(x = x, y = ylin)) + 
  geom_point(alpha = 0.5, size = 3) + 
  stat_smooth(method = "lm", se = FALSE, color = 'darkred', lwd = 1.5) + 
  labs(x = "Feature", y = "Label") + config
```
...
```{r, echo = FALSE}
pr <- 1/(1+exp(-ylin))
ylog <- rbinom(200, 1, pr)

log_df <- data.frame(y = ylog, x = x)
ggplot(log_df, aes(x = x, y = ylog)) + 
  geom_point(alpha = 0.5, size = 3) + 
  stat_smooth(method = "glm", method.args = list(family=binomial), se = FALSE, color = 'darkred', lwd = 1.5) + 
  labs(x = "Feature", y = "Label") + config
```
]
...
.bg-light-green.b--dark-green.ba.bw1.br3.pl4[Ridge, lasso, and elastic net penalties can also be added to the logistic loss function.

These will have the same effect of .imp[shrinking coefficients towards zero].
]
---
class: onecol
