...
.footnote[*Note.* The selector function `starts\_with()` allows you to easily capture all dummy codes for a variable.]
...
.imp[Centering] a predictor involves changing its mean to $0.0$

- This is accomplished by subtracting the mean from every observation 

--

.imp[Scaling] a predictor involves changing its standard deviation (and variance) to $1.0$

- This is accomplished by dividing each observation by the standard deviation

--

.imp[Normalizing] a predictor involves centering it and then scaling it
...
```{r yjfare, echo = FALSE}
yj_train <- bake(yj_recipe %>% prep(titanic), new_data = titanic)
p1 <- 
...
- This typically means adding .imp[nonlinear expansions] of existing predictors<sup>1</sup>
...
```{r poly, echo=FALSE}
nl_train <- bake(nl_recipe %>% prep(titanic), new_data = titanic)
...
```{r, eval=FALSE}
library(correlation)
correlation(mc_titanic) %>% filter(abs(r) > 0.75)
```

```{r, echo=FALSE}
library(correlation)
correlation(mc_titanic) %>% filter(abs(r) > 0.75) %>% print_md()
```
