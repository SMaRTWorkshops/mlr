...
date: 'Day 1-PM1 &emsp; &emsp; August 8, 2022'
output:
...
---
class: twocol
# Why is data spending so important?
...
.right-column[
Observations in multilevel data are not independent. 

Random resampling of rows may cause biased train/test sets.

E.g., patients from hospital 1 may end up mostly in the training set, while patients from hospital 2 are primarily in the test set.

When working with multilevel data, resampling should occur at the level of the experimental unit.
]
...
We can't use data from the future to predict the past! 

Rather, it is typical to use recent data for a test set. 
...
---
class: onecol
# Preview of Advanced Resampling Methods

Today we reviewed the rationale for **separating model training from model testing**. 

We learned various methods for holdout cross-validation: 
- Simple random split
- Stratified sampling
- Validation set
- Multilevel data
...
Tomorrow will cover more advanced state-of-the-art methods for **k-fold cross-validation** and **nested cross-validation**.
